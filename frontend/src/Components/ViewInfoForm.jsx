...
function ViewInfoForm({ info }) {
  const express_time = new Date(info.express_time * 1000)
    .toISOString()
    .slice(0, 16);
  const expiration_time = new Date(info.expiration_time * 1000)
    .toISOString()
    .slice(0, 16);
  const create_time = new Date(info.created_at * 1000)
    .toISOString()
    .slice(0, 16);
  return (
    <>
