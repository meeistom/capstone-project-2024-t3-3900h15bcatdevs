  ...
  it("opens each patient and displays complete milk entry details", () => {
    cy.get("table tbody tr").each(($row) => {
      cy.wrap($row)
        ...
        cy.get("select").eq(1).select(newAdditiveType); // Additive Type
        cy.get("select").eq(2).select(newStorageType); // Storage Type
        cy.get(".col").contains("Storage Location").next().find("input").clear().type(newStorageLocation);
        ...
        cy.contains("Milk Details").parent().within(() => {
          ...
          cy.get("select").eq(1).should("have.value", newAdditiveType.toLowerCase());
          cy.get("select").eq(2).should("have.value", newStorageType);
