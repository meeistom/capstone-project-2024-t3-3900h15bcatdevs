...
def get_new_milk_uid(firestore_client) -> Tuple[bool, str]:
    ...
    if not firestore_client.collection('stats').document('milk_stats').get().exists:
        return False, "Error, stats document does not exist"

    stats_collection = firestore_client.collection('stats')
    uid_tracker_document = stats_collection.document('milk_stats').get().to_dict()
    uid_tracker = uid_tracker_document['uid_counter'] # Currently the largest UID in use
    ...
    stats_collection.document('milk_stats').update({
        'uid_counter': firestore.Increment(1)
    })
